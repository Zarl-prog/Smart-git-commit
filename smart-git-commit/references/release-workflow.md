...
echo "## Performance"
git log $(git describe --tags --abbrev=0)..HEAD --oneline --grep="^perf"

echo "## Security"
git log $(git describe --tags --abbrev=0)..HEAD --oneline --grep="^security"
```
...
# If features only: bump MINOR
# If fixes/chores only: bump PATCH
```
...
git commit -m "release: bump version to v1.x.x" \
  -m "See CHANGELOG.md for full list of changes."
```
...
alias release-patch='npm version patch && git push --follow-tags'
alias release-minor='npm version minor && git push --follow-tags'
alias release-major='npm version major && git push --follow-tags'

# Generate changelog from last 3 tags
git log --oneline --no-decorate v1.0.0..v1.1.0

# Show commits by author since last release
